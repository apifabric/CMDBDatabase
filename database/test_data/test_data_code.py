...
while restart_count < 5 and has_errors:
    ...
    try:
        if not 3077795775329257429 in succeeded_hashes:  # avoid duplicate inserts
            instance = server1 = Server(hostname="server01", ip_address="192.168.1.1", location_id=1)
            session.add(instance)
            session.commit()
            succeeded_hashes.add(3077795775329257429)
    except Exception as e:
        has_errors = True
    ...
    try:
        if not -2703967103076892872 in succeeded_hashes:  # avoid duplicate inserts
            instance = server2 = Server(hostname="server02", ip_address="192.168.1.2", location_id=2)
            session.add(instance)
            session.commit()
            succeeded_hashes.add(-2703967103076892872)
    except Exception as e:
        has_errors = True
        if 'UNIQUE' in str(e) and restart_count > 1:
            pass
    ...
    try:
        if not 8235884139395063758 in succeeded_hashes:  # avoid duplicate inserts
            instance = server3 = Server(hostname="server03", ip_address="192.168.1.3", location_id=1)
            session.add(instance)
            session.commit()
            succeeded_hashes.add(8235884139395063758)
    except Exception as e:
        has_errors = True
        if 'UNIQUE' in str(e) and restart_count > 1:
            pass
    ...
    try:
        if not 4472490483088442204 in succeeded_hashes:  # avoid duplicate inserts
            instance = server4 = Server(hostname="server04", ip_address="192.168.1.4", location_id=2)
            session.add(instance)
            session.commit()
            succeeded_hashes.add(4472490483088442204)
    except Exception as e:
        has_errors = True
        if 'UNIQUE' in str(e) and restart_count > 1:
            pass
    ...
    try:
        if not -7971033073854022573 in succeeded_hashes:  # avoid duplicate inserts
            instance = location1 = Location(name="DataCenter 1", address="123 Main St.")
            session.add(instance)
            session.commit()
            succeeded_hashes.add(-7971033073854022573)
    except Exception as e:
        has_errors = True
        if 'UNIQUE' in str(e) and restart_count > 1:
            pass
    ...
    try:
        if not 4523468229447614483 in succeeded_hashes:  # avoid duplicate inserts
            instance = location2 = Location(name="DataCenter 2", address="456 Side St.")
            session.add(instance)
            session.commit()
            succeeded_hashes.add(4523468229447614483)
    except Exception as e:
        has_errors = True
        if 'UNIQUE' in str(e) and restart_count > 1:
            pass
    ...
    try:
        if not 2661022593099238755 in succeeded_hashes:  # avoid duplicate inserts
            instance = location3 = Location(name="Remote Storage", address="789 Remote Rd.")
            session.add(instance)
            session.commit()
            succeeded_hashes.add(2661022593099238755)
    except Exception as e:
        has_errors = True
        if 'UNIQUE' in str(e) and restart_count > 1:
            pass
    ...
    try:
        if not 5811755251674521363 in succeeded_hashes:  # avoid duplicate inserts
            instance = location4 = Location(name="HQ Office", address="10 HQ Blvd.")
            session.add(instance)
            session.commit()
            succeeded_hashes.add(5811755251674521363)
    except Exception as e:
        has_errors = True
        if 'UNIQUE' in str(e) and restart_count > 1:
            pass
